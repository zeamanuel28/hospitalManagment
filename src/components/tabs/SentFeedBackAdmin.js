...
const SentFeedBackAdmin = ({fecth}) => {
  const [loading, setLoading] = useState (false);
  const [sentData, setSentData] = useState ([]);

  const getSentFeedBackAdmin = async () => {
    setLoading (true);
    try {
      const res = await axios.get (`/api/sms/feedback/sent`);
      setLoading (false);
...
export default SentFeedBackAdmin